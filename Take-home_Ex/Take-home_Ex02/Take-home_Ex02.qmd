...
This box plot helps to understand the variability of prices among the different property types. An interesting insight about the price patterns among different floor categeries also has been derived out.
...
1.  Reversing the x and y axes for the box plot will be useful as the categories on the x-axis can then be numerical values and the lengthy labels can be on the y axis making it more readable.
...
3.  Changed the order of Floor category types to appear in the order of low,medium and high.

4.  Tried using varwidth=TRUE, for the box plots to reflect the actual count of data points,but this didnt look visually appealing as some property types like Terraced house,semi-detatched house and detatched house has very less data points resulting in extremely narrow boxes making it difficult to see the median value.

    Also tried using fixed width size by setting width = 0.5 for the box plot. But this resulted in the fixed width being split among the different floor categories for a property type and Terraced house,semi-detatched house and detatched house to have a fixed width, which again looked odd and not representative of the actual data.

    Finally, used position_dodge2(preserve = "single") so that all boxes including the boxes corresponding to the different floor categories are of same size. This removed the distraction away from different sized box plots which are not representative of the data.
